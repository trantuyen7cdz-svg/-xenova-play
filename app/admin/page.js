...
export default function AdminPage() {
  ...
  async function checkAdmin() {
    try {
      ...
      console.error(error);
      setMessage("Có lỗi xảy ra khi kiểm tra quyền Admin.");
      setLoading(false);
    }
  }
  ...
  async function saveProduct(event) {
    ...
    if (editingId) {
      ...
      if (error) {
        setMessage(
          "Không thêm được sản phẩm: " + error.message
        );
        return;
      }
  ...
  async function approveOrder(order) {
    const ok = window.confirm(
      `Duyệt đơn #${order.id} với số tiền ${Number(
        order.amount || 0
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        setMessage(
          result.message || "Không duyệt được đơn."
        );
      ...
      setMessage(
        `Đã duyệt đơn #${order.id} và cấp KEY ${result.key.key_code}`
      );
  ...
  async function rejectOrder(order) {
    const ok = window.confirm(
      `Bạn có chắc muốn từ chối đơn #${order.id} không?`
    );
    ...
    if (error) {
      setMessage(
        "Không từ chối được đơn: " + error.message
      );
    ...
    setMessage(`Đã từ chối đơn #${order.id}.`);
    ...
    <main className={styles.adminPage}>
      <div className={styles.adminContainer}>
        ...
        >
          ...
          <p className={styles.sectionText}>
            KEY sẽ được tự động tạo khi Admin duyệt
            đơn hàng.
          </p>
